package com.trongthang.survivaloverhaul.fluid;
...
public abstract class PurifiedWaterFluid extends FlowableFluid {
    ...
    @Override
    public void randomDisplayTick(World world, BlockPos pos, FluidState state,
        ...
        if (!state.isStill() && !state.get(FALLING).booleanValue()) {
            if (random.nextInt(64) == 0) {
                world.playSound((double) pos.getX() + 0.5, (double) pos.getY() + 0.5, (double) pos.getZ() + 0.5,
                        SoundEvents.BLOCK_WATER_AMBIENT, net.minecraft.sound.SoundCategory.BLOCKS,
    ...
    public BlockState toBlockState(FluidState state) {
        return ModBlocks.PURIFIED_WATER.getDefaultState().with(FluidBlock.LEVEL, getBlockStateLevel(state));
    }
